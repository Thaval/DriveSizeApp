# DriveSizeApp

This repository hosts the **release builds** for DriveSize.

- Releases: https://github.com/Thaval/DriveSizeApp/releases

## Downloads

Go to the Releases page and download the build that matches your system:

- `DriveSize-win-x64-selfcontained.zip` (Windows x64, no .NET install required)
- `DriveSize-win-arm64-selfcontained.zip` (Windows ARM64, no .NET install required)

## Screenshot

![DriveSize – Start](https://github.com/Thaval/DriveSize/blob/master/Assets/ScreenshotStart.png)

## How to Run

1. Download the ZIP from Releases
2. Extract it
3. Run `DriveSize.exe`

## Notes

These builds are **self-contained** and include the .NET runtime, so they run on any supported Windows 10/11 machine without installing .NET.
